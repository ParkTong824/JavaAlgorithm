...
public class S160_IntersectionOfTwoLinkedLists {
    ...
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        List<ListNode> aNodeList = new ArrayList<>();
        List<ListNode> bNodeList = new ArrayList<>();
        aNodeList.add(headA);
        bNodeList.add(headB);
        if (aNodeList.contains(headB)){
            return headB;
        } else if( bNodeList.contains(headA)) {
            return headA;
        }
    ...
    public List<Integer> makeCheckList (ListNode node) {
        List<Integer> checkList = new ArrayList<>();
        ListNode temp = node;
        checkList.add(temp.val);
